...
#define pin_LED_rojo A2 // Pin LED rojo
#include <EnableInterrupt.h>

// PWM
float ESC1_us, ESC2_us, ESC3_us, ESC4_us,ESC5_us, ESC6_us,loop_timer;
...
void setup() {
  ...
  pinMode(pin_motor5, OUTPUT);   // Declarar motor 3
  pinMode(pin_motor6, OUTPUT);   // Declarar motor 4
  ...
  digitalWrite(pin_motor5, LOW); // Motor 3 LOW por seguridad
  digitalWrite(pin_motor6, LOW); // Motor 4 LOW por seguridad
...
void loop() {
  ...
  digitalWrite(pin_motor5, HIGH);  // MOTOR 3
  digitalWrite(pin_motor6, HIGH);  // MOTOR 4
  ...
  while (digitalRead(pin_motor1) == HIGH || digitalRead(pin_motor2) == HIGH || digitalRead(pin_motor3) == HIGH || digitalRead(pin_motor4) == HIGH || digitalRead(pin_motor5) == HIGH || digitalRead(pin_motor6) == HIGH) {
    ...
    if (tiempo_motores_start + ESC5_us <= micros()) digitalWrite(pin_motor5, LOW); // MOTOR 3
    if (tiempo_motores_start + ESC6_us <= micros()) digitalWrite(pin_motor6, LOW); // MOTOR 4
  }
