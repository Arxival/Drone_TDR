...
#define pin_motor5 11       // Pin motor 5
#define pin_motor6 10       // Pin motor 6
...
#define buzzer A2    // Pin LED rojo 2    
...
int PID_W_sat1   = 40;  // Limitar parte integral PID velocidad
int PID_W_sat2   = 40;  // Limitar salida del PID velocidad
int PID_ang_sat1 = 16;  // Limitar parte integral PID ángulo
int PID_ang_sat2 = 16;  // Limitar salida del PID ángulo
...
/// SEÑALES PWM
float ESC1_us, ESC2_us, ESC3_us, ESC4_us, ESC5_us, ESC6_us;
...
volatile unsigned long Throttle_HIGH_us;
volatile unsigned int RC_Throttle_raw;
void INT_Throttle() {
...
volatile unsigned long Pitch_HIGH_us;
volatile unsigned int RC_Pitch_raw;
void INT_Pitch() {
...
volatile unsigned long Pitch_Roll_us;
volatile unsigned int RC_Roll_raw;
void INT_Roll() {
...
volatile unsigned long Pitch_Yaw_us;
volatile unsigned int RC_Yaw_raw;
void INT_Yaw() {
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("Setup-Start next Wire.begin();");
  // Iniciar I2C
  Wire.begin();
  ...
  Serial.println("començem buzzer");
  // Declaración de LEDs
  pinMode(buzzer, OUTPUT); // Led naranja --> Batería baja
                                     
                                  
  pinMode(buzzer, OUTPUT);   // Led rojo 2  --> Tiempo de ciclo o de 1ms excedido
  ...
  pinMode(pin_motor5, OUTPUT);  //Motor 5
  pinMode(pin_motor6, OUTPUT);  //Motor 6
  // Forzar los pines a estado LOW
  digitalWrite(pin_motor1, LOW);
  digitalWrite(pin_motor2, LOW);
  digitalWrite(pin_motor3, LOW);
  digitalWrite(pin_motor4, LOW);
  digitalWrite(pin_motor5, LOW);
  digitalWrite(pin_motor6, LOW);
  ...
  Serial.println ("iniciem MPU");
  MPU6050_iniciar();
  MPU6050_calibrar();
  Serial.println ("Llegim bateria");
...
void loop() {
  ...
  Serial.println("guardar lectures");
...
void RC_procesar() {
  ...
  interrupts();
  // Si las lecturas son cercanas a 0, las forzamos a 0 para evitar inclinar el drone por error (us)
...
void Modulador() {
  ...
  if (RC_Throttle_consigna <= 1300) {
    ...
    ESC1_us = RC_Throttle_consigna*0.125;
    ESC2_us = RC_Throttle_consigna*0.125;
    ESC3_us = RC_Throttle_consigna*0.125;
    ESC4_us = RC_Throttle_consigna*0.125;
    ESC5_us = RC_Throttle_consigna*0.125;
    ESC6_us = RC_Throttle_consigna*0.125;

    // Si lo motores giran con el stick de Throttle al mínimo, recudir el valor de 950us
    if (ESC1_us < 1000) ESC1_us = 950*0.125;
    if (ESC2_us < 1000) ESC2_us = 950*0.125;
    if (ESC3_us < 1000) ESC3_us = 950*0.125;
    if (ESC4_us < 1000) ESC4_us = 950*0.125;
    if (ESC5_us < 1000) ESC5_us = 950*0.125;
    if (ESC6_us < 1000) ESC6_us = 950*0.125;
  }
  ...
  // Si el throttle es mayor a 1300us, el control de estabilidad se activa.
    ...
    // Modulador
    ESC1_us = (RC_Throttle_consigna - PID_W_Roll_OUT - PID_W_Yaw_OUT)*0.125; // Motor 1
    ESC2_us = (RC_Throttle_consigna - PID_W_Pitch_OUT - PID_W_Roll_OUT + PID_W_Yaw_OUT)*0.125; // Motor 2
    ESC3_us = (RC_Throttle_consigna - PID_W_Pitch_OUT + PID_W_Roll_OUT - PID_W_Yaw_OUT)*0.125; // Motor 3
    ESC4_us = (RC_Throttle_consigna + PID_W_Roll_OUT + PID_W_Yaw_OUT)*0.125; // Motor 4
    ESC5_us = (RC_Throttle_consigna + PID_W_Pitch_OUT + PID_W_Roll_OUT - PID_W_Yaw_OUT)*0.125; // Motor 5
    ESC6_us = (RC_Throttle_consigna + PID_W_Pitch_OUT - PID_W_Roll_OUT + PID_W_Yaw_OUT)*0.125; // Motor 6
    ...
    if (ESC1_us < 1100) ESC1_us = 1100*0.125;
    if (ESC2_us < 1100) ESC2_us = 1100*0.125;
    if (ESC3_us < 1100) ESC3_us = 1100*0.125;
    if (ESC4_us < 1100) ESC4_us = 1100*0.125;
    if (ESC5_us < 1100) ESC5_us = 1100*0.125;
    if (ESC6_us < 1100) ESC6_us = 1100*0.125;
    // Evitamos mandar consignas mayores a 2000us a los motores
    if (ESC1_us > 2000) ESC1_us = 2000*0.125;
    if (ESC2_us > 2000) ESC2_us = 2000*0.125;
    if (ESC3_us > 2000) ESC3_us = 2000*0.125;
    if (ESC4_us > 2000) ESC4_us = 2000*0.125;
    if (ESC5_us > 2000) ESC5_us = 2000*0.125;
    if (ESC6_us > 2000) ESC6_us = 2000*0.125;
  }
}
...
void PWM() {
  ...
  digitalWrite(pin_motor5, HIGH);
  digitalWrite(pin_motor6, HIGH);
  ...
  while (digitalRead(pin_motor1) == HIGH || digitalRead(pin_motor2) == HIGH || digitalRead(pin_motor3) == HIGH || digitalRead(pin_motor4) == HIGH || digitalRead(pin_motor5) == HIGH || digitalRead(pin_motor6) == HIGH) {
    ...
    if (tiempo_motores_start + ESC5_us <= micros()) digitalWrite(pin_motor5, LOW);
    if (tiempo_motores_start + ESC6_us <= micros()) digitalWrite(pin_motor6, LOW);
 }
}
...
void Lectura_tension_bateria() {
  ...
  //Consigo que el contador cada 1s se actualize independiente del estado de la bateria
  if (millis() == LOW_BAT_WARING_timer);
    LOW_BAT_WARING_timer += 1000;
  if (RC_Throttle_consigna < 1300) {
...
void Visualizaciones() {
  ...
  if (visu == 1) {
    if (visu_select == 0) {
      Serial.println(RC_Pitch_consigna);
      Serial.println("\t");
    ...
    if (visu_select == 1) {
      Serial.println((gx - gyro_X_cal) / 16.4);
      Serial.println("\t");
      Serial.println((gy - gyro_Y_cal) / 16.4);
      Serial.println("\t");
      Serial.println((gz - gyro_Z_cal) / 16.4);
    }

    if (visu_select == 2) {
      Serial.println(ax / 4096);
      Serial.println("\t");
      Serial.println(ay / 4096);
      Serial.println("\t");
    ...
    if (visu_select == 4) {
      Serial.println(ESC1_us);
      Serial.println("\t");
      Serial.println(ESC2_us);
      Serial.println("\t");
      Serial.println(ESC3_us);
      Serial.println("\t");
