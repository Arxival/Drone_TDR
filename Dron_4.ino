...
#define pin_distanciax
#define pin_distanciay
#define pin_distanciaz
...
#define pin_LED_rojo2 A1    // Pin LED rojo 2    
#define pin_LED_naranja A2  // Pin LED naranja    
#define trigg A3            //Trigger
#define echo A4             //Echo
// --------------------------------------------------------------------------------
...
// interrupció del echo
volatile long distancia
volatile long temps_distancia
volatile long temps_trigger
void medir_distancia{
  if (digitalRead(echo)==HIGH);{
    temps_echo = micros()
  }
  if (digitalRead(echo)==LOW);{
    temps_distancia=micros()- temps_echo;
    distancia = temps_distancia*0.000343;
    comptador_repeticions = 0
  }
}
void setup() {
  // Iniciar I2C y LCD
  ...
  enableInterrupt(echo, medir_distancia, CHANGE);
  ...
  digitalWrite(pin_LED_rojo2, 255);
  lcd.setCursor(0, 0);
...
void loop() {
  ...
  if (micros() - loop_timer > usCiclo + 50) digitalWrite(pin_LED_rojo2, 255);
...
void Modulador() {
  ...
  if (RC_Throttle_consigna <= 1300) {
    ...
    ESC5_us = RC_Throttle_consigna;
    ESC6_us = RC_Throttle_consigna;
    ...
    if (ESC5_us < 1000) ESC3_us = 950;
    if (ESC6_us < 1000) ESC4_us = 950;
  }
  ...
  // Si el throttle es mayor a 1300us, el control de estabilidad se activa.
    ...
    // Modulador
    ESC1_us = RC_Throttle_consigna - PID_W_Roll_OUT - PID_W_Yaw_OUT; // Motor 1
    ...
    ESC4_us = RC_Throttle_consigna + PID_W_Roll_OUT + PID_W_Yaw_OUT; // Motor 4
    ESC1_us = RC_Throttle_consigna + PID_W_Pitch_OUT + PID_W_Roll_OUT - PID_W_Yaw_OUT; // Motor 5
    ESC2_us = RC_Throttle_consigna + PID_W_Pitch_OUT - PID_W_Roll_OUT + PID_W_Yaw_OUT; // Motor 6
    ...
    if (ESC5_us < 1100) ESC5_us = 1100;
    if (ESC6_us < 1100) ESC6_us = 1100;
    ...
    if (ESC5_us > 2000) ESC3_us = 2000;
    if (ESC6_us > 2000) ESC4_us = 2000;
  }
}
...
void PWM() {
  ...
  if (tiempo_ON > 900) digitalWrite(pin_LED_rojo2, 255);
  // ------------------ ¡¡1ms max!! ------------------
...
void LED_blink() {
  if (LED_contador % 20 == 0) {
    ...
    analogWrite(pin_LED_naranja,0);
  }
  LED_contador++;
}
void enviar_senyal {
  if (comptador_repeticions = 0);
    comptador_rep = 1
    temps_trigger = micros ()
    digitalWrite(trigger,HIGH)
    delayMicroseconds(10)
    digitalWrite(trigger,LOW)
}
void processar_senyal_ultrasons {
  if (distancia != consigna_x){
    if (distancia > consigna_x){
      RC_Pitch_consigna += 200
    }
    else {
      RC_Pitch_consigna -= 200
    }
...
void LIDAR {
      if(TFmini.measure()){                      //Measure Distance and get signal strength
        distancia_y = TFmini.getDistance();       //Get distance data
      }
      if(distancia_y != 0){
        if(distancia_y < consigna_y){
          RC_throttle_consigna += 200
        }
        else{
          RC_throttle_consigna -= 200
        }
